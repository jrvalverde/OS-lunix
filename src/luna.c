...
void Howto()
{
  printf("Luna 6502/10-cross-assembler version 1.23\n");
  printf("Usage:\n");
  printf("  luna [-jlLoOpq] sourcefile\n");
...
int insert_label(char *labname, int *pos)
{
  ...
  while (labname[lowerb]!='\0') {
    ...
    if (label_tab_pos+lowerb>=label_max*label_len) {
...
#     endif
    ...
    if (final_pass && !quiet_mode && (label_stat[i]&fl_used)==0 \
...
void main(int argc, char **argv)
{
  ...
  if (str_cmp(file_input,file_output)) {
    ...
    if (pre_proccess)
      ...
      /* Make lupo command */
      sprintf(temp,"lupo -ql %s -o %s",file_input,pre_file);
			  ...
			  while(1)
				{
				  switch(line[j])
					{
					case '\"': /* string" */
					  for(i=(j+1);i<(nextsep(j)-1);i++) /* skip the quotes */
						{
						  putbyte(line[i],
								  fl_resolved,0); /* resolved, no label */
						}
					  break;
					default: /* expression */
					  j=getexpr(j,&ltmp,&flags,&lab);
				  ...
				  /* to the next field */
				  j=nextchar(j);
				  endOfField=nextsep(j);
				  if (line[endOfField]!=',')
					{
					  j=nextsep(nextchar(j));
					  break;
					}
				}
			  continue;
